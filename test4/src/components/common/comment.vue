...
<script>
    export default {
        ...
        //先调用这个函数
        loadFirst(){
            // +'?pageindex=1'[this.cid]
            this.$axios.get('src/getcomments.json')
            .then(res=>{
        ...
        sendComment(){
            //发表评论请求后台
            //this.$axios.post('src/postcomment.json'+ this.cid,'content='+this.msg)
            this.$axios.get('src/postcomment.json','content='+this.msg,{headers:{'Content-Type':'application/x-www-form-urlencoded'}})
            .then(res=>{
                console.log(res);
                //获取最新的第一页数据
                this.loadFirst();
